#!/usr/bin/env python3
"""Hayabusa 9B vs Ollama 9B 同条件ベンチマーク.

同一モデルサイズ(Qwen3.5-9B)で純粋なサーバー性能を比較する。
- Hayabusa: Qwen3.5-9B-Q4_K_M.gguf (llama.cpp + KVキャッシュスロット)
- Ollama:   qwen3.5:9b

Usage:
    # Hayabusa起動後に実行
    python bench_9b_vs_9b.py

    # サンプル数変更
    python bench_9b_vs_9b.py --samples 50
"""
...
# 固定プロンプト群（短・中・長）— bench_compare.py と同一
PROMPTS = [
    # Short
...
OUTPUT_PATH = SCRIPT_DIR / "bench_results_9b_vs_9b.json"
...
def print_header():
    print()
    print("=" * 95)
    print("  Hayabusa (9B Q4_K_M, llama.cpp) vs Ollama (qwen3.5:9b)")
    print("  max_tokens=%d  temperature=%d  prompts=%d" % (MAX_TOKENS, TEMPERATURE, len(PROMPTS)))
    print("=" * 95)
...
def print_comparison(results: list[BenchResult]):
    ...
    h_data = by_target.get("hayabusa", {})
    o_data = by_target.get("ollama", {})
    if not h_data or not o_data:
        return

    print()
    print("== 同条件比較 (Qwen3.5-9B) ==")
    print()
    row_fmt = "  {conc:>5}   {h_lat:>8.0f}  {o_lat:>8.0f}  {lat_x:>6.2f}x   {h_tok:>7.1f}  {o_tok:>7.1f}  {tok_x:>6.2f}x   {h_p95:>8.0f}  {o_p95:>8.0f}"
    print("   Conc    Hay(ms)  Oll(ms)  Lat倍率    Hay t/s  Oll t/s  tok倍率    Hay P95  Oll P95")
    print("  " + "-" * 88)
    ...
    for c in concurrencies:
        ...
        if not h or not o:
            continue
        lat_x = o.avg_latency / h.avg_latency if h.avg_latency > 0 else 0
        tok_x = h.tok_per_sec / o.tok_per_sec if o.tok_per_sec > 0 else 0
        print(row_fmt.format(
            conc=c, h_lat=h.avg_latency, o_lat=o.avg_latency, lat_x=lat_x,
            h_tok=h.tok_per_sec, o_tok=o.tok_per_sec, tok_x=tok_x,
            h_p95=h.p95, o_p95=o.p95,
        ))
    print()
...
async def main_async(args):
    ...
    if len(available) < 2:
        ...
        if "hayabusa" in missing:
            print("  Start: .build/debug/Hayabusa models/Qwen3.5-9B-Q4_K_M.gguf --slots 4")
        if "ollama" in missing:
            print("  Ensure: ollama serve is running and qwen3.5:9b is pulled")
        sys.exit(1)

    print_header()
    print(f"  Samples per level: {num_samples}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Hayabusa 9B vs Ollama 9B benchmark (同条件比較)")
