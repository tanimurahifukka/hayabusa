...
// ── Connections（HAYABUSAなし — Claudeから直接ツリー） ──
const CONNECTIONS = [
  // Claude → Classify
  { from: 'claude',   to: 'classify',  color: '#ff9f43' },
  // Claude → Generalists
  { from: 'claude',   to: 'qwen',      color: '#00d4ff' },
  { from: 'claude',   to: 'gemma',     color: '#00d4ff' },
  // Qwen → Specialists
  ...
  try {
    ...
    for (const ev of events) {
      ...
      if (ev.type === 'request') {
        ...
        setTimeout(() => {
          sendPulse('claude', 'qwen', '#00d4ff', 500);
        }, 500);
      }
      ...
      if (ev.type === 'completion') {
        ...
        sendPulse('qwen', 'claude', '#00ff88', 500);
