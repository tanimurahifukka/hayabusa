...
async function pollEvents() {
  try {
    ...
    for (const ev of events) {
      ...
      if (ev.type === 'request') {
        // リクエスト開始: Claude → Hayabusa → Qwen パルス
        ...
        // パルスアニメーション
        sendPulse('claude', 'hayabusa', '#e94560', 500);
        setTimeout(() => {
          sendPulse('hayabusa', 'qwen', '#00d4ff', 400);
        }, 300);
      }

      if (ev.type === 'completion') {
        // 完了: Qwen → Hayabusa → Claude パルス（結果返却）
        ...
        sendPulse('qwen', 'hayabusa', '#00ff88', 400);
        setTimeout(() => {
    ...
    if (statusEl) {
      ...
      statusEl.textContent = Object.keys(pendingRequests).length > 0 ? 'PROCESSING' : 'ONLINE';
    }
  } catch {
    const statusEl = nodeElements['hayabusa']?.querySelector('.status');
